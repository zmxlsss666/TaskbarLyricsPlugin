...
object ConfigManager {
    ...
    fun initialize() {
        try {
            spwConfigManager = WorkshopApi.instance.manager.createConfigManager(PLUGIN_ID)
            configHelper = spwConfigManager?.getConfig()
            
            spwConfigManager?.addConfigChangeListener { helper ->
                println("检测到配置变更，重新加载配置")
                reloadFromDisk()
    ...
    private fun saveToHardcodedPath() {
        try {
            val configFile = File(getHardcodedConfigPath())
            ...
            val jsonContent = buildString {
                append("{\n")
                append("  \"font_family\": \"${getFontFamily()}\",\n")
                append("  \"font_size\": ${getFontSize()},\n")
                append("  \"font_color\": \"${getFontColor()}\",\n")
                append("  \"background_color\": \"${getBackgroundColor()}\",\n")
                append("  \"alignment\": \"${getAlignment()}\",\n")
                append("  \"show_translation\": ${showTranslation()}\n")
                append("}")
            }
            ...
            println("配置已保存到文件: ${configFile.absolutePath}")
        } catch (e: Exception) {
            println("保存配置到文件失败: ${e.message}")
        }
    }
    ...
    fun applyConfig() {
        ...
        configHelper?.let { helper ->
            try {
                helper.set(KEY_FONT_FAMILY, getFontFamily())
                helper.set(KEY_FONT_SIZE, getFontSize())
                helper.set(KEY_FONT_COLOR, getFontColor())
                helper.set(KEY_BACKGROUND_COLOR, getBackgroundColor())
                helper.set(KEY_ALIGNMENT, getAlignment())
                helper.set(KEY_SHOW_TRANSLATION, showTranslation())
                
                if (helper.save()) {
                    println("配置已成功保存到SPW配置系统")
                    saveToHardcodedPath()
                    reloadFromDisk()
                    notifyConfigChangeListeners()
                    
                    try {
                        WorkshopApi.instance.ui.toast("配置已成功应用", WorkshopApi.Ui.ToastType.Success)
                    } catch (e: Exception) {
                        println("发送通知失败: ${e.message}")
                    }
                } else {
                    println("保存配置到SPW系统失败")
                    try {
                        WorkshopApi.instance.ui.toast("保存配置失败", WorkshopApi.Ui.ToastType.Error)
                    } catch (e: Exception) {
                        println("发送通知失败: ${e.message}")
                    }
                }
                ...
                e.printStackTrace()
                try {
                    WorkshopApi.instance.ui.toast("应用配置时出错: ${e.message}", WorkshopApi.Ui.ToastType.Error)
                } catch (ex: Exception) {
        ...
        configHelper?.let { helper ->
            try {
                ...
                if (helper.save()) {
                    ...
                    try {
                        WorkshopApi.instance.ui.toast("配置已重置为默认值", WorkshopApi.Ui.ToastType.Success)
                    } catch (e: Exception) {
                        println("发送通知失败: ${e.message}")
                    }
                } else {
                    println("重置配置失败")
                    try {
                        WorkshopApi.instance.ui.toast("重置配置失败", WorkshopApi.Ui.ToastType.Error)
                    } catch (e: Exception) {
                        println("发送通知失败: ${e.message}")
                    }
                }
                ...
                try {
                    WorkshopApi.instance.ui.toast("重置配置时出错: ${e.message}", WorkshopApi.Ui.ToastType.Error)
                } catch (ex: Exception) {
                    println("发送通知失败: ${ex.message}")
                }
            }
